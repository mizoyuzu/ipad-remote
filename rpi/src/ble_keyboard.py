"""BLE keyboard HID report sender."""
...
class BleKeyboardHID:
    """
    Sends 8-byte keyboard HID reports over BLE GATT notifications.

    Report wire format: ``\\xa1 <report_id> <8-byte USB-format report>``

    Presents the same write / release / close interface as KeyboardHID and
    BtKeyboardHID so it can be swapped in wherever those are used.
    """
